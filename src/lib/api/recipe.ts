...
import type { Recipe, RecipeCategory, RecipeDiet, RecipeUnit } from "@/types/recipe";
...
export async function getRecipes(options: GetRecipesOptions = {}): Promise<PaginatedResponse<Recipe>> {
  ...
  if (options.filters) {
    ...
    if (options.filters.diets) {
      options.filters.diets.forEach(dietId => params.append('diets[]', dietId.toString()));
    }
  }

  const response = await axiosClient.get<PaginatedResponse<Recipe>>(`/recipes`, { params });
...
export async function updateRecipe(id: number, data: FormData): Promise<Recipe> {
  ...
  const response = await axiosClient.post<{ data: Recipe }>(`/recipes/${id}`, data, {
    headers: {
      'Content-Type': 'multipart/form-data',
    },
  });
...
export async function getMyRecipes(page = 1, limit = 10): Promise<PaginatedResponse<Recipe>> {
  const params = { page: page.toString(), per_page: limit.toString() };
  const response = await axiosClient.get<PaginatedResponse<Recipe>>('/recipes/my', { params });
  return response.data;
}

export async function getFavoriteRecipes(page = 1, limit = 10): Promise<PaginatedResponse<Recipe>> {
  const params = { page: page.toString(), per_page: limit.toString() };
  const response = await axiosClient.get<PaginatedResponse<Recipe>>('/recipes/favorites', { params });
  return response.data;
}

export async function getRecipeCategories(): Promise<RecipeCategory[]> {
  const response = await axiosClient.get<{ data: RecipeCategory[] }>('/recipe-categories');
  return response.data.data;
}

export async function getRecipeDiets(): Promise<RecipeDiet[]> {
  const response = await axiosClient.get<{ data: RecipeDiet[] }>('/recipe-diets');
  return response.data.data;
}

export async function getRecipeUnits(): Promise<RecipeUnit[]> {
  const response = await axiosClient.get<{ data: RecipeUnit[] }>('/recipe-units');
  return response.data.data;
}