...
function PasswordField({ id, label, placeholder, register, errors }: PasswordFieldProps) {
    ...
    return (
        <div className="space-y-2">
            ...
            {error && <p className="text-sm text-destructive mt-1" role="alert">{error.message}</p>}
        </div>
    );
}
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <Input id="name" type="text" placeholder="Seu nome completo" {...register('name')} className="pl-10" aria-invalid={!!errors.name} />
                            </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <Input id="email" type="email" placeholder="seu@email.com" {...register('email')} className="pl-10" aria-invalid={!!errors.email} />
                            </div>
                            {errors.email && <p className="text-sm text-destructive mt-1" role="alert">{errors.email.message}</p>}
                        </div>

                        <PasswordField id="password" label="Senha" placeholder="Mínimo 8 caracteres" register={register} errors={errors} />
                        <PasswordField id="confirmPassword" label="Confirmar senha" placeholder="Confirme sua senha" register={register} errors={errors} />
                        ...
                        <div className="flex items-start space-x-2">
                            <Controller
                                ...
                                control={control}
                                render={({ field }) => (
                                    <Checkbox id="terms" checked={field.value} onCheckedChange={field.onChange} className="mt-1" aria-invalid={!!errors.acceptTerms} />
                                )}
                            />
                            <div className="grid gap-1.5 leading-none">
                                ...
                                {errors.acceptTerms && <p className="text-sm text-destructive" role="alert">{errors.acceptTerms.message}</p>}
                            </div>
                        </div>
